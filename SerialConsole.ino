...
void Help() { //HELP
  ...
  Serial.println("time\tDisplay the seconds since boot.");
  Serial.println("");
}
...
/*void Time(){  //TIME
  String MINUTEs="-m"
  while (iteration <= Input.length() && (Input.charAt(iteration) == HELp.charAt(iteration) || Input.charAt(iteration) == ))
  if (iteration == TIMe.length() && (! Input.charAt(iteration + 1) || Input.charAt(TIMe.length() + 1) == 32))
    ...
    if (incomingString.indexOf(13) >= 0) {  //ENTER
      ...
      if (Input.length()==0) {  /*If input IS empty*/
        ...
        while (iteration <= Input.length() && Input.charAt(iteration) == HELp.charAt(iteration)) {  /*Checks for length and order*/
          if (iteration == HELp.length() && ((! Input.charAt(iteration + 1)) || (Input.charAt(iteration + 1) == 32))) {  /*If input IS 'help' OR 'help '*/
            Help();
            command=1;
          }
        ...
        while (iteration <= Input.length() && Input.charAt(iteration) == CLEAr.charAt(iteration)) {  /*Checks for length and order*/
          if (iteration == CLEAr.length() && ((! Input.charAt(iteration + 1)) || Input.charAt(iteration + 1) == 32)) {  /*If input IS 'clear' OR 'clear '*/
            Clear();
            command=1;
          }
          iteration=iteration + 1;
        ...
        while (iteration <= Input.length() && Input.charAt(iteration) == TIMe.charAt(iteration)) {  /*Checks for length and order*/
          if (iteration == TIMe.length() && (! Input.charAt(iteration + 1) || Input.charAt(TIMe.length() + 1) == 32)) {  /*If input IS 'time' OR 'time '*/
  //          Time();
            command=1;
          }
          iteration=iteration + 1;
    ...
    while (Input.indexOf("\x1B[A") >= 0 || Input.indexOf("\x1B[B") >= 0 || Input.indexOf("\x1B[C") >= 0 || Input.indexOf("\x1B[D") >= 0 || Input.indexOf(127) >= 0 || Input.indexOf(9) >= 0) {
      if (Input.indexOf(127) >= 0) {
        Input.remove(Input.indexOf(127), 1);
      }
      ...
      if (Input.indexOf(9) >= 0) {
        Input.remove(Input.indexOf(9), 1);
      }
      iteration=iteration + 1;
    }
