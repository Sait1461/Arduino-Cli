...
//Use "ctrl+a" and then "\" to exit screen
...
void Help() { //HELP
  ...
  Serial.println("reboot\tReboots the board.");
...
void Time(){  //TIME
  if (Input.length() > Commands[4].length() && Input.charAt(Input.indexOf(45) + 1) == 109) {//Minutes '-m'
...
void COMMANDS(){
  ...
  if (Input.length()==0) {  /*If input IS empty*/
    ...
    while (iteration < sizeof(Commands)/sizeof(Commands[0]) && ! command) {
      ...
      if (i == Commands[iteration].length() && (! Input.charAt(i) || Input.indexOf(32) == Commands[iteration].length())) {  /*If input IS '[COMMAND]' OR '[COMMAND] '(with a space)*/
      command=true;
      All();
      }
      iteration++;
